...
class DataAnalyzer:
    ...
    def on_connect(self, client, userdata, flags, rc):
        self.logger.info(
            "Connected successfully to broker, response code {}".format(rc)
        )

    def on_disconnect(self, client, userdata, rc):
        if rc != 0:
            self.logger.warning(
                "Connection ended unexpectedly from broker, error code {}".format(rc)
            )
    ...
    def on_message(self, client, userdata, message):
        ...
        try:
            ...
            self.logger.error(
                "An error ocurred while hanlding new message of {}: {}".format(
                    message.topic, err
                )
            )
